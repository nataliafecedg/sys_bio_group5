...
```{r}
# --- Save melanoma objects ---
stats_deseq_mel <- stats_deseq
stats_dex_mel   <- stats_dex
fg_deseq_mel    <- fg_deseq
fg_dex_mel      <- fg_dex
sig_deseq_mel   <- sig_deseq
sig_dex_mel     <- sig_dex
de_df_mel       <- de_df
dx_df_mel       <- dx_df
cmp_mel         <- cmp
spearman_cor_mel <- spearman_cor
```
...
```{r}
# --- Save bladder objects ---
stats_deseq_bl <- stats_deseq
stats_dex_bl   <- stats_dex
fg_deseq_bl    <- fg_deseq
fg_dex_bl      <- fg_dex
sig_deseq_bl   <- sig_deseq
sig_dex_bl     <- sig_dex
de_df_bl       <- de_df
dx_df_bl       <- dx_df
cmp_bl         <- cmp
spearman_cor_bl <- spearman_cor

rm(stats_deseq, stats_dex, fg_deseq, fg_dex, sig_deseq, sig_dex,
   de_df, dx_df, cmp, spearman_cor)
```
