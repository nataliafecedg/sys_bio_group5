...
plotPCA(vsd_melanoma, intgroup = "Groups")
```
...
res_melanoma <- results(dds_melanoma)
summary(res_melanoma)
sum(res_melanoma$padj < 0.05, na.rm=TRUE) # number of significant genes
```
...
plotPCA(vsd_bladder, intgroup = "Groups")

dds_bladder <- DESeq(dds_bladder)
res_bladder <- results(dds_bladder)
summary(res_bladder)
sum(res_bladder$padj < 0.05, na.rm=TRUE) # number of significant genes
...
myStats_melanoma <- res_melanoma[,"stat"]
names(myStats_melanoma) <- rownames(res_melanoma)
...
```{r}
# 2. Make ranked stats vector (use Ensembl IDs as names)
myStats_bladder <- res_bladder[,"stat"]
names(myStats_bladder) <- rownames(res_bladder)
...
# plot enrichment
plotEnrichment(BP_list[[chosen_pathway_melanoma]], myStats_melanoma) +
...
# plot enrichment
plotEnrichment(BP_list[[chosen_pathway_bladder]], myStats_bladder) +
...
library(ggrepel)
ggplot(fg_volcano_melanoma, aes(x = NES, y = mlog10padj, color = sig)) +
...
ggplot(fg_volcano_bladder, aes(x = NES, y = mlog10padj, color = sig)) +
  geom_point(alpha = 0.8) +
  geom_hline(yintercept = -log10(0.05), linetype = 2) +
  geom_text_repel(data = to_label,
                  aes(label = stringr::str_trunc(pathway, 50)),
