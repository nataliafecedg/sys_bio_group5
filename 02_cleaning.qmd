...
Before filtering, we perform a sanity check to ensure that the count matrices contain no **NA values**, **negative numbers**, or **non-integer entries**, which would indicate corrupted or improperly loaded data. If any anomalies are detected, the script stops to preserve the **quality** and **reliability** of the downstream analysis.
...
## Creating copies of metadata for later

```{r}
bladder_metadata <- read_delim("bladder_metadata.csv", delim = ";")
melanoma_metadata <- read_delim("melanoma_metadata.csv", delim = ";")

bladder_metadata_orig  <- bladder_metadata
melanoma_metadata_orig <- melanoma_metadata
